...
<template>
  <div class="resetPassword">
    ...
    <div class="text-item error-item">
      <v-icon 
        type="icon-login-error" :size="14" v-if="errorFlag"></v-icon>
      {{passwordText}}
    </div>
    <!-- 密码 -->
    <div class="form-item">
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    beforeRouteLeave: function(to, from, next) {
      ...
      sessionStorageUtil.remove('temp-kinder-forget-phone')
      sessionStorageUtil.remove('temp-kinder-forget-code')
      next()
    },
    methods: {
      /** 
      ...
      _getSessionInfo: function() {
        this.passwordForm = {
          phone: sessionStorageUtil.get('temp-kinder-forget-phone'),
          code: sessionStorageUtil.get('temp-kinder-forget-code'),
          password: '',
          confirmPassword: ''
        }
      },
      ...
      _handleModifyPassword: function(param) {
        return requestModifyPassword(param).then( response => {
          if (response.code == 200) {
            this.passwordText = ''
            this.errorFlag = false
            localStorageUtil.set('kinder-userName', this.passwordForm.phone)
            localStorageUtil.set('kinder-password', this.passwordForm.password)
